...
public class HangerIOReal implements HangerIO {
  ...
  public HangerIOReal(int id, boolean isclockwice_Positive) {
    // 假设 Hanger 电机 ID 为 15，位于 CANivore ("rio" 或你的 CANbus 名称)
    talon = new TalonFX(HangerConstants.kHangerId);
    ...
    final TalonFXConfiguration config =
        new TalonFXConfiguration()
            ...
            .withFeedback(
                new FeedbackConfigs().withSensorToMechanismRatio(HangerConstants.kHangerGearRatio))
            .withCurrentLimits(
            ...
            .withSoftwareLimitSwitch(
                new SoftwareLimitSwitchConfigs()
                    ...
                    .withForwardSoftLimitThreshold(
                        Units.radiansToRotations(HangerConstants.kHangerMaxAngle)) // radian转圈数
                    .withReverseSoftLimitEnable(true)
                    .withReverseSoftLimitThreshold(
                        Units.radiansToRotations(HangerConstants.kHangerMinAngle)))
            .withMotionMagic(
                new MotionMagicConfigs()
                    .withMotionMagicCruiseVelocity(
                        Units.radiansToRotations(HangerConstants.kVelocityRadPerSec))
                    .withMotionMagicAcceleration(
                        Units.radiansToRotations(HangerConstants.kAccelerationRadPerSecSq)));

    talon.getConfigurator().apply(config);

    resetPosition(HangerConstants.kHangerInitialAngle);
  ...
  @Override
  public void applyOutputs(HangerIOOutputs outputs) {
    switch (outputs.mode) {
