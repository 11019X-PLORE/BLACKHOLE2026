...
public class LCPCeP extends SequentialCommandGroup {
  public LCPCeP(RobotContainer c, Supplier<Boolean> isRightSide) {
    Arm arm = c.arm;
    ...
    addCommands(
        ...
        drive.resetOdomToPath("LCP"),
        Commands.deadline(
            drive.generatePath("LCP"),
        ...
        Commands.deadline(
            drive.generatePath("PCeP"),
            blocker.setGoalCommand(BlockerGoal.HALFOPEN),
            arm.setGoalCommand(ArmGoal.INTAKE),
            indexer.setGoalCommand(IndexerGoal.INTAKE),
            flywheel.setGoalCommand(FlywheelGoal.INTAKE)),
        Commands.deadline(new WaitCommand(4), CommandFactory.passing(c), blocker.shootCommand()));
  }
}
