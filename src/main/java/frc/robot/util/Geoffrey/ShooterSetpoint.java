...
public class ShooterSetpoint {
    ...
    public ShooterSetpoint(double shooterVelocityMetersPerSec, double shooterAccelerationMetersPerSecSquared, double turretPositionRadians, double turretVelocityRadsPerSec, double turretAccelerationRadsPerSecSquared,
            double hoodPositionRadians, double hoodVelocityRadsPerSec, double hoodAccelerationRadsPerSecSquared) {
     ...
     public static ShooterSetpoint makeSetpoint(
        ...
        Translation3d vBallField = new Translation3d(vH_req * angleToTarget.getCos(), vH_req * angleToTarget.getSin(), vZ_req);
        ...
        Translation3d aBallField = new Translation3d(
        ...
        // Position & Velocity
        double turretPos = MathUtil.angleModulus(vLaunch.toTranslation2d().getAngle().minus(muzzleJoint.getGlobalPose().getRotation().toRotation2d()).getRadians());
        ...
        double alphaField = ( (0 - 0) * denT - (x * vy - y * vx) * (2 * x * vx + 2 * y * vy) ) / (denT * denT);
        ...
        double alphaHood = ( (0 - 0) * denH - (h * vz - z * vh) * (2 * h * vh + 2 * z * vz) ) / (denH * denH);
        ...
        return new ShooterSetpoint(
            ...
            turretPos, turretVel, turretAccel,
            hoodPos, hoodVel, hoodAccel
        );
    }
}
